...
- Added `sentinella/core/broadcast.py`, isolating WebSocket fan-out and connection lifecycle management into `BroadcastHub`. Features per-client send timeouts (`WS_SEND_TIMEOUT`), a concurrent client cap (`MAX_WEBSOCKET_CLIENTS`), and a resilient broadcast loop that survives individual failed cycles without terminating the streaming server.
...
- Added `sentinella/web/static/lib.js` as a UMD module containing pure formatting, sorting, rate calculation, container status checks, and key obfuscation utilities shared between the Web Dashboard and the Vitest test suite without requiring a build step.
- Added wire snapshot trimming via `serialize_snapshot()` in `sentinella/core/api.py`: drops unrendered `cmdline` data (~70% payload reduction) and caps processes to `max_display` for browser dashboards and WebSocket frames, while introducing `?full=true` for remote CLI clients needing complete exports.
...
- Expanded the `MetricCollector` protocol (`sentinella/core/protocols.py`) to include `active_modules`, `last_collected_at`, and `collect_module_async`. Updated `RemoteCollector` (`sentinella/remote/client.py`) to satisfy this full protocol, automatically discovering `active_modules` via upstream `/health` and querying `?full=true` snapshots.
- Added remote agent support to `sentinella fetch` (`sentinella --remote <addr> fetch`), displaying a live formatted console summary of a remote host.
- Added `RemoteUnavailable` error handling in `sentinella/cli.py`, providing actionable diagnostics (such as prompting for an API key on HTTP 401) instead of raw tracebacks when remote agents fail or reject connections.
- Added CLI module validation in `sentinella print --modules`, rejecting unrecognized module names with a list of valid modules rather than silently outputting nothing.
- Added CLI guard preventing `--remote` usage with `sentinella web` and `sentinella serve`, surfacing an informative error explaining that server commands must run directly on the host being monitored.
...
- Added `ContainerInfo.is_running` property and `RUNNING_STATUSES` constant in `sentinella/core/models.py` to standardize container running state detection across the TUI, Web Dashboard, and console fetch summary.
...
- Reordered HTTP middleware in `sentinella/core/api.py` so security headers (`X-Content-Type-Options`, `X-Frame-Options`, `Content-Security-Policy`) are applied outermost, guaranteeing they are attached even when requests fail authentication with HTTP 401.
- Expanded `_is_public_bind` in `sentinella/core/api.py` using Python's `ipaddress` module to detect IPv6 wildcards (`::`), empty strings, and explicit non-loopback addresses, ensuring `warn_open_bind` triggers for all insecure interfaces without an API key.
...
- Rich console markup in `sentinella/fetch.py` is now escaped with `rich.markup.escape()` across user names, hostnames, OS info, mountpoints, interface names, and sensor labels to prevent markup parsing errors.
...
- Fixed `Collector._inflight` task management in `sentinella/core/collector.py`: completed futures are no longer reused on subsequent cycles (which previously replayed stale data and halved effective refresh rate), and finished tasks are pruned in a `finally` block to prevent pinning completed future objects.
...
- Fixed plugin loader `_load_plugin_module` in `sentinella/core/plugin_manager.py` to only load `MonitorPlugin` subclasses defined directly within the target module (`obj.__module__ == fqn`), preventing imported sibling plugin classes from being loaded mistakenly.
...
- Fixed palette memory leak in `sentinella/tui/theme.py`: converted the module-level `_cache` into a `weakref.WeakKeyDictionary` keyed by app instance.
...
- Added `sentinella/tui/theme.py`, resolving Textual design tokens (`$text-primary`, `$text-success`, …) into concrete colours for dashboard widgets. Rich `Text` styles cannot name a token — `Static.update()` parses them through Rich's colour parser — so the token classes in `dashboard.tcss` could never have applied to inline spans.
- Added `sentinella/core/limits.py` as the single source of truth for how many rows each surface shows (`DASHBOARD_LIMITS` for the TUI panels and web cards, `EXPORT_LIMITS` for `sentinella print`), replacing inline slice caps in the widgets and the text exporter.
...
- Added `level_for_percent` and `level_for_temp` to `sentinella/core/utils.py`, separating threshold logic from colour choice so the TUI, console output, and web dashboard can share thresholds while rendering them differently.
- Added `truncate_path` to `sentinella/core/utils.py`, truncating from the left so sibling mount paths stay distinguishable.
...
- Web Dashboard process sorting now follows `sentinella/core/sort.py`: the same key names, field mapping, case-insensitive name comparison, and per-key default direction (PID and Name ascend; CPU, MEM, RSS, and Threads descend) instead of always starting descending.
...
- An unknown battery charge now shows as "Unknown" in the Web Dashboard rather than a critical-coloured 0%, matching the TUI and `sentinella fetch`.
...
- Added a warning (log entry and stderr message) when a loaded `sentinella.toml` contains a plaintext `api_key` but the file is group/world-readable, prompting the user to `chmod 600` it.
...
- Centralized process sort-key definitions into a shared `sentinella/core/sort.py` module, replacing duplicated sort maps in the exporters, `ProcessesPlugin`, and `ProcessTable`.
...
- Fixed a potential `RuntimeError: Set changed size during iteration` in `sentinella/core/api.py` by copying the active WebSockets set prior to broadcasting.
...
- Added proper TUI teardown handling in `SentinellaApp.on_unmount` to close the collector and release resources on exit.
...
- Added background server orchestration in `sentinella/core/runner.py` to daemonize web and remote server execution, decoupling uvicorn thread spawning from CLI routing.
...
- Improved configuration parser in `sentinella/config.py` to gracefully fallback to default sub-configurations and warn when scalar values are supplied for configuration sections.
...
- Standardized package name as `sentinella-monitor` across `pyproject.toml`, imports, and metadata.
